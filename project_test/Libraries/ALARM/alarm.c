...
void Alarm_Process(void) {
    ...
    if (last_state != *p_alarm_state) {
        if (*p_alarm_state == STATE_WAIT_FOR_RAIN) {
            // [수정 1] 빗물 감지 대기 화면 메시지 변경
            LCD_Clear(YELLOW);
            // 여러 줄로 나누어 출력 (좌표 조정)
            LCD_ShowString(10, 40, (u8*)"Waiting for water...", BLACK, YELLOW);
            LCD_ShowString(10, 70, (u8*)"Go to bathroom", BLACK, YELLOW);
            LCD_ShowString(10, 90, (u8*)"right now!!", BLACK, YELLOW);
            LCD_ShowString(10, 120, (u8*)"Don't fall asleep", BLACK, YELLOW);
            LCD_ShowString(10, 140, (u8*)"again!", BLACK, YELLOW);
        ...
        case STATE_WAIT_FOR_RAIN:
            Motor_Stop();
            {
                ...
                if (abs((int)rain_val - (int)last_rain_val) > 50) {
                    // [수정 2] Rain Sensor -> Water Sensor 로 변경
                    sprintf(lcd_buffer, "Water Sensor: %04d", rain_val);
                    // 위쪽 텍스트가 많아졌으므로 Y좌표를 150 -> 170으로 내림
                    LCD_ShowString(20, 170, (u8*)lcd_buffer, BLACK, YELLOW);
