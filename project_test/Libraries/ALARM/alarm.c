    ...
    switch (*p_alarm_state) {
        case STATE_COUNTDOWN:
            if (last_displayed_second != (int32_t)*p_countdown_seconds) {
                ...
                sprintf(lcd_buffer, " %02d  %02d", (int)(*p_countdown_seconds / 60), (int)(*p_countdown_seconds % 60));
        ...
        case STATE_ALARM_STOPPED:
            sprintf(lcd_buffer, "%d %d", (int)(*p_elapsed_seconds / 60), (int)(*p_elapsed_seconds % 60));
            LCD_Clear(WHITE);
...
void Alarm_Reset(void) {
    ...
    *p_countdown_seconds = 0;
    ...
    BUZZER_Off(); // 부저 끄기
    LCD_Clear(WHITE);
    LCD_ShowString(40, 100, (u8*)"waiting", BLUE, WHITE);
}