...
// --- 상태별 처리 함수들 ---
static void ProcessState_Countdown(int32_t* last_sec, 
                                    uint32_t* dist_L, uint32_t* dist_C, uint32_t* dist_R,
                                    uint32_t* last_dist_L, uint32_t* last_dist_C, uint32_t* last_dist_R) {
    char lcd_buffer[30];
    char time_str[20];
...
static void ProcessState_AlarmActive(uint32_t* dist_L, uint32_t* dist_C, uint32_t* dist_R,
                                     uint32_t* last_dist_L, uint32_t* last_dist_C, uint32_t* last_dist_R,
                                     char* last_action) {
    char lcd_buffer[30];
...
static void ProcessState_WaitForRain(uint32_t* stability_count) {
    char lcd_buffer[30];
...
static void ProcessState_AlarmStopped(int32_t* last_sec) {
    char lcd_buffer[30];
    char time_str[20];
...
static void ProcessState_Idle(uint32_t* sensor_timer,
                              uint32_t* dist_L, uint32_t* dist_C, uint32_t* dist_R) {
    char lcd_buffer[30];
...
static void HandleStateTransition(AlarmState* last_state, uint32_t* last_dist_L, char* last_action, uint32_t* stability_count) {
    if (*last_state == *p_alarm_state) return;
...
void Alarm_Process(void) {
    ...
    // 상태 변경 시 초기화 로직 실행
    HandleStateTransition(&last_state, &last_dist_L, last_action, &stability_count);

    // 상태별 처리
    switch (*p_alarm_state) {
        case STATE_COUNTDOWN:
            ProcessState_Countdown(&last_sec, &dist_L, &dist_C, &dist_R, 
                                   &last_dist_L, &last_dist_C, &last_dist_R);
            break;

        case STATE_ALARM_ACTIVE:
            ProcessState_AlarmActive(&dist_L, &dist_C, &dist_R,
                                    &last_dist_L, &last_dist_C, &last_dist_R,
                                    last_action);
            break;

        case STATE_WAIT_FOR_RAIN:
            ProcessState_WaitForRain(&stability_count);
            break;

        case STATE_ALARM_STOPPED:
            ProcessState_AlarmStopped(&last_sec);
            break;

        case STATE_IDLE:
            ProcessState_Idle(&sensor_timer, &dist_L, &dist_C, &dist_R);
            break;
    }
}
